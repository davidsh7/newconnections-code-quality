...
/**
 * Regla: no-unsupported-polyfills
 *
 * Detecta el uso de APIs de Node.js que requieren polyfills en el browser
 * y verifica que estén configurados en vite.config.mts (via alias manual
 * o via vite-plugin-node-polyfills).
 */
...
const POLYFILL_APIS: Record<string, PolyfillConfig> = {
  assert: {
  ...
  string_decoder: {
    message:
      "Node.js string_decoder API requires polyfill in browser environment",
    patterns: [
...
function checkViteConfig(cwd: string): Set<string> {
  const viteConfigPath = path.join(cwd, "vite.config.mts");
  const configuredPolyfills = new Set<string>();
...
function isWebCryptoOnly(text: string): boolean {
...
function matchesText(text: string, pattern: RegExp | string): boolean {
...
function parseNodePolyfillsPlugin(
  content: string,
  configuredPolyfills: Set<string>,
): void {
  const includeMatch =
    /nodePolyfills\s*\(\s*\{[^\}]*include\s*:\s*\[(?<list>[^\]]+)\]/sv.exec(
      content,
    );
...
const noUnsupportedPolyfills: Rule.RuleModule = {
  create(context) {
  ...
  meta: {
    docs: {
      ...
      description:
        "Disallow Node.js APIs that require polyfills without proper configuration",
      recommended: true,
    },
    schema: [],
    type: "problem",
